---
title: "Microbiome Guide"
output: 
  html_document: 
    theme: yeti
---
<div style="text-align:center;"><h1><b> Microbiome Data</b></h1>  
The following guide explains how to operate within the Microbiome Data module of EcoPLOT. Here we explain the capabilities unique to this module.</div>  

<h2><b>What is Microbiome Data?</b></h2>  
  
  Microbiome data is used to understand the microbial composition within in a community sample. This is often done via amplicon analyses, in which a specific piece of DNA is amplified many times over via various methods (i.e. PCR). The DNA piece, referred to as the amplicon, is sequenced and compared with a database to determine the host's identity. Researchers commonly generate 16S amplicons. As an essential gene, the 16S region is slow to undergo mutations, but exhibits enough variability between microbial species that it is often used as a unique identifier of species identity. Amplicon counts of each species are collected within a metadata table that is to be interpreted by the researcher. 
  
  The Microbiome Data module of EcoPLOT is well equipped to assist in your analysis of microbial communities from any ecosystem. 
  
<hr>

<h2><b>Microbiome Data Format Guide</b></h2>  
...
<h2><b>Filtering of Data</b></h2>  
  ...
  Users can access both the original and filtered datasets across module sections. 
...
<h2><b>Community Composition</b></h2>  
  ...
  **Note:** Lower taxonomic ranks may cause the plot legend to obstruct the plot itself. You may alter the position of the legend using the legend position option.  
  ...
  If a phylogenetic tree was uploaded, you may plot it in the "Phylogenetic Tree" tab. Plotting options are taken directly from the uploaded file and may take a bit to load. You may select which node and how many ancestral levels to show. 
  
  <hr>
  
<h2><b>Alpha Diversity</b></h2>  
  
  Alpha diversity measurements are used to estimate the diversity within a single sample. Alpha diversity focuses on observing species <b>evenness</b>, how similar each microbe is distributed within a sample, and <b>richness</b>, the number of different species present. In EcoPLOT, we provide users with a table of standard alpha diversity estimators (Observed ASV's, Chao1, ACE, Shannon, Simpson, Inverse Simpson, and Fisher). These methods can then be viewed graphically in the following tab, titled "Plot." <br> Below we explain the differences between each method so that you may determine which best suits your data:  
  <ul>
...
<li><b>Fisher:</b>This method assumes that the species abundance and total number of species follow a log scale. This method places a large importance on rare species.</li>
  ...
  The statistics tool provided in the sidebar of this tab will inform users of statistically significant differences between experimental design variables as given by a Tukey Pairwise Comparison. Users are asked to select which variables they would like to compare. Once selected, a print-out will show the pairwise comparisons and their corresponding significance. 
